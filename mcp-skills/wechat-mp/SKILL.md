...
- **已内置重试与统一错误**：网关 `callApi` 对网络错误 / 超时 / 5xx 自动重试 2 次（4xx 不重试）；失败返回统一结构 `{ "error": { "type": "network"|"timeout"|"http", "status"?, "message": "..." } }`。AI 只需判断返回文本中是否含 `error.type` 即可判定失败，并按 `type` 区分处理，**无需在 skill 层手动重试**。
