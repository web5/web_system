...
Page({
  ...
  askFollowUp(e: any) {
    ...
    if (!question) return;
    wx.navigateTo({
      url: `/packageContract/pages/contract/chat/chat?question=${encodeURIComponent(question)}${this.conversationQuery()}`,
    });
  },

  /** 通用"追问"按钮：进入空白对话页（仅展示分析结论作为第一轮，由用户自行输入提问） */
  goChat() {
    wx.navigateTo({ url: `/packageContract/pages/contract/chat/chat${this.conversationQuery(true)}` });
  },
