...
新增 `p-bind-email`（自绘二级页）+ 个人信息页邮箱三态，按同一判据源复验：

- 形态正确：收邮箱**未使用 `wx.showModal`**（原生弹窗无输入框），改自绘二级页 —— 判据 **I5**（原生能力不伪装）。
- 字段级校验：邮箱格式、6 位验证码均为字段级红色提示，不清空输入 —— 判据 **H4**、checklist D。
- 限频可见：发送后 60s 倒计时且按钮不可点 —— 判据 **P3**、**S8**。
- 冲突合并：邮箱已注册走同一套合并（`bindSource='email'`），文案写明「合并不可撤销」—— 判据 **S6** + 一致性（与手机号同一流程，不为邮箱写第二套）。
- 脱敏展示：`a***@example.com`，与手机号「只展示脱敏形式」口径一致 —— requirements §3。
- 无裸 hex、SVG 无 emoji、token 一致 —— **G1 / X5**。
