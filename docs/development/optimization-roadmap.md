...
- **问题**：`apps/kedou-ai-minigram`、`apps/deploy-console`、`servers/system-service`、`servers/todo-service`、`servers/deploy-console` 子目录残留 npm 的 `package-lock.json`，在 pnpm workspace 中混用 npm 会破坏依赖一致性。
- **执行**：
...
- **问题**：测试几乎为零：
  ...
  - 前端 5 个应用中仅 kedou-ai-minigram（3 个）、portal（1 个）有测试
- **执行**（按优先级）：
