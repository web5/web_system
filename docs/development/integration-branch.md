...
| 跳过模块 | 默认 `mini-contract`（其 build 是上传微信平台，本地必失败）；`--skip-modules a,b` 覆盖。注：历史上还跳过 `finnews`（仓库无目录），该模块已并入 content-hub 并清理 |
...
| `--skip-modules` | `mini-contract` | 本地不参与自动发布的模块（逗号分隔） |
