...
| 跳过模块 | 默认 `finnews,kedou-ai-minigram,mini-contract`（本地必失败：`finnews` 仓库无目录、`kedou-ai-minigram` 与 `mini-contract` 的 build 是上传微信平台需密钥）；`--skip-modules a,b` 覆盖。注：`finnews` 已并入 content-hub 被清理，保留仅为兼容旧环境 |
...
| `--skip-modules` | `finnews,kedou-ai-minigram,mini-contract` | 本地不参与自动发布的模块（逗号分隔） |
