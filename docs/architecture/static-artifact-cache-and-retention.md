...
| 2026-09-13 | **结论推翻** | 查 dev 流水线 cleanup 命令实为"**跳过历史版本清理**" → **T2 不成立**（那 5 个版本是全部历史，非保留窗口）；改为 **T1 变体**（同版本目录被平台外投递覆盖）；并明确 **B1 在当前配置下不触发**。§10.4 整节重写 |
...
### 10.4 触发条件定性（2026-09-13 修正）

> ⚠️ 本节在 2026-09-13 被推翻重写：上一版判断"**T2 是主凶、B1 是对症主药**"，经查 dev 的流水线 cleanup 命令后**不成立**。

**T2（旧版本被 cleanup 清理）→ 不成立。**
dev 的 `deploy_pipeline_step_commands` 里 cleanup 节点的命令内容就是：

```bash
#!/usr/bin/env bash
echo "[pipeline:cleanup] 跳过历史版本清理（避免批量删除审批；如需清理请在流水线编辑页配置）"
```

即 **cleanup 从不执行**。旁证：dev 上 admin 现存 **6 个**版本目录（`10b74d8 12e8c60 41fffaa 4caf272 ba44c25 f05e12e`），与历史发布一一对应，从未被清理。原版文档"5 个 = KEEP_VERSIONS=5"的吻合是**巧合**。

**T1（同一 commit 重复投递覆盖）→ 变体成立。**

- dev 平台自身的 `deploy_pipelines` 里 admin 只发过 4 次（`ba44c25` / `10b74d8` / `12e8c60` / `f05e12e`），且**无软删记录**（`soft_deleted = 0`）；
- 但 `12e8c60` 目录里的主 chunk 已从 `main.DLkP3_K.js` 变为 `main.DLIcP3_K.js`，而 `DLkP3_K` 在 dev 任何现存目录中都找不到 → **该目录被覆盖过**；
- 覆盖者来自**平台之外的投递**：dev 的流水线模板创建于 2026-09-11（id 时间戳），此前是人工/脚本部署；且 `upload` 节点命令对同版本会先 `[ -d "$ARTIFACT_DIR" ] && mv "$ARTIFACT_DIR" /tmp/upload-...`，**同版本重投会把旧产物整体移走**。

**结论**：**A1（入口 no-cache）才是对症的修复** —— 它保证浏览器始终拿到"当前存在"的入口，不再被缓存的旧入口引向已消失的分包。

**关于 B1 的适用性（重要）**
`cleanup` 节点在两端都被配置成 **script + "跳过清理"**，因此平台内置的 `CleanupExecutor`（B1 所在代码）**根本不会被调用** —— B1 在当前配置下**不触发**。它的价值是"当 cleanup 节点被清空、回退内置逻辑时，24h 下限立即生效"。若希望 B1 真正起作用，需在流水线编辑页把 cleanup 节点的命令**留空**（回退平台内置清理）。
