...
- [ ] **mac release 同步**：`cd ~/web_system_release && git fetch && git merge --ff-only origin/master`
...
- [ ] **腾讯云安全组**放通 prod `6000-6007`（来自 SSL 层 + 公网）
- [ ] **业务侧已通知停机窗口**
...
- [ ] 8 服务 dist：`rsync -az --delete servers/<svc>/dist/ kedou-prod:/data/web_system/servers/<svc>/dist/`
- [ ] workspace 包：`rsync -az packages/{shared,types,agent-core,...}/dist/` 与 `package.json`

---

## C. prod 端依赖（5.3-5.6，最易出错）

- [ ] `pnpm install --prefer-offline --no-frozen-lockfile --shamefully-hoist`（必须带 `--shamefully-hoist`）
- [ ] 同步 `.pnpm` 内 workspace 包副本（`shared`/`types`/`agent-core`/...）
- [ ] 手动补 pnpm 漏装的包（`ws`/`adm-zip`/...）从 mac rsync
- [ ] workspace 包名兼容软链：`/data/web_system/node_modules/@kedouai/agent-core → packages/agent-core`

---

## D. 服务 .env 补丁（5.7）

- [ ] `system-service`：加 `AUTH_SERVICE_URL=http://127.0.0.1:6001`（生产 fail-fast 必填）
- [ ] 其他服务：扫描代码里 `REQUIRED_SERVICE_URLS_IN_PROD` 声明的 URL 全部填齐

---

## E. ecosystem 端口切 600x（5.8）

- [ ] `sed` 改 `PORT: 300x` → `PORT: 600x`
...
- [ ] SSH `42.194.200.69` 备份 nginx conf
- [ ] `sed` 改 `proxy_pass http://106.52.176.246:3000` → `:6000`
- [ ] `sudo nginx -t && sudo systemctl reload nginx`
- [ ] `sudo nginx -T | grep "proxy_pass http://106"` 确认已是 `6000`
...
- [ ] `https://kedouai.com/api/health` → 200
- [ ] `https://admin.kedouai.com/` → 200
- [ ] `POST https://kedouai.com/api/ai/chat` → 200（业务核心）
- [ ] `pm2 save` 完成
...
| 公网访问报错 | 先看 SSL 层 nginx error log：`ssh 42.194.200.69 'sudo tail /var/log/nginx/error.log'` |
| 服务日志 | `ssh kedou-prod "pm2 logs <svc> --lines 200"` |
| 数据库直查 | `mysql -hgz-cdb-8y2lp8rt.sql.tencentcdb.com -P27241 -uroot web_system_deploy` |
