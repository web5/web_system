...
deploy_frontend() { # $1=module_name
  ...
  if [ "$mod" = "shell" ]; then
    ...
    if [ "$DRY_RUN" != "1" ]; then
      ...
      remote "cd $REMOTE_DIR/servers/gateway/public/shell && tar xzf /tmp/shell-deploy.tar.gz && { find assets -type f -mtime +7 -delete 2>/dev/null || true; } && rm -f /tmp/shell-deploy.tar.gz"
