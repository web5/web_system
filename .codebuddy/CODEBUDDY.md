...
| 小程序 | 微信原生 + TS | `apps/kedou-ai-minigram` |
...
```
web_system/
├── apps/        # 前端：shell(基座) admin portal kedou-ai-minigram(小程序) deploy-console(独立 SPA)
...
```
前端 apps/（shell 基座 + portal/admin 模块 + kedou-ai-minigram + deploy-console SPA）
...
| kedou-ai-minigram | apps/kedou-ai-minigram | 微信小程序（科豆 AI） | 独立上传 |
...
- 变变产品（暖色）：主色 `#FF8C42` 魔法橙 / 底色 `#FFF8F0` / 文字 `#333333` —— 品牌色只用于 portal/kedou-ai-minigram，不套用 admin UI 规范（DR-5）
