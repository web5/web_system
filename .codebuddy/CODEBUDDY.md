...
### 2.5.3 独立代码评审（新增对外接口 · 与 UI 无关）

凡改动**新增对外接口**（`*.controller.ts` / `*.controller.js` 为新增文件）：

按 `docs/development/code-review-checklist.md` 过一遍 → 出评审报告 → **阻塞项清零** → commit 带 `Code: pass`（或报告路径；CI R15 拦截）。

- 判据源：`docs/development/code-review-checklist.md`（A 接口入参 / B 错误处理 / C 安全 / D 可观测 / E 资源副作用 / F 一致性）
- 承担角色：`code-reviewer`（**独立于实现者自查** `rd-review`；不改码，只出报告并回流）
- **只查机器查不出的**：R1–R8 已覆盖调试残留 / 占位 / TS 铁律 / 硬编码凭据 / CORS 等 —— **不重复查**，否则评审者做机械工作
- 纯微调走 `Micro-exempt: <理由>`

> 为什么新增接口要单独设门：**越权、重复提交、失败静默、敏感泄露**这四类问题机器查不出，而它们恰好集中在「新增一个对外接口」时引入。
> 与 `rd-review`（实现者自查）的分工：自查的天然缺陷是**知道自己想做什么**，于是会替代码补上不存在的意图；独立评审的价值就在于**不知道**。
