...
- 不适用：portal / kedou-ai-minigram（品牌端 DR-5，Claymorphism，走 `docs/ui/ui-design-spec.html`，不套本卡）
...
| 形态 | 容器 | 适用 | 基准 |
|------|------|------|------|
| 移动端 | `.phone` | 品牌/产品/C 端（portal/kedou-ai-minigram） | 390px |
...
- **主色三态**：admin 系平台主橙 `#F97316`（DR-3）；品牌端换产品品牌色（如科豆 AI 小程序 `#F97316`）。
...
- admin 系落地 Vue3 + Ant Design Vue；品牌端落 portal / kedou-ai-minigram（Claymorphism，`ui-design-spec.html`，不套本卡）。
...
- 不套本规范到品牌端（portal/kedou-ai-minigram）。
- 微前端改动不执行"四步"不宣布完成。
