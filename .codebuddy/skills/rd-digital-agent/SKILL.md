...
| 契约评审 | `contract-reviewer`（项目可选装配） | 跨端/跨服务**契约变更**的独立第三方评审：按项目契约登记（接口 / 事件 / 工具 / 权限码 / 路由）查**消费方影响面**，判「改一处会不会让消费方静默失效」；破坏性变更须附消费方清单。不代改、不代登记 |
| 交付评审 | `release-reviewer`（项目可选装配） | 交付环节独立第三方评审：按项目发布判据清单评审发布 / 环境 / 数据变更，判「**测试通过 ≠ 能上线**」（改的不在被加载的目录 / 配置源被会话污染 / 端口被旧进程占用 / 迁移落错库）。不执行发布、不改码 |
...
模型是「**环节 × 角色**」而**不是一条链**：每个环节内并列「生产者 / 自查（下限）/ 独立评审（第三方）」，
另有 **4 个人审节点固定**：需求定稿 → 方案与交付物确认 → 原型确认 → **放行上线**。

环节骨架（完整环节表 / 回流边 / 分级 / 各评审的通用契约，见 [`references/review-chain.md`](references/review-chain.md)）：

| 环节 | 做什么 | 自查（下限） | 独立评审 |
|---|---|---|---|
| S0 需求 | 需求 spec（验收判据 / 反例） | — | 判据可测性（`test-verification` 前置）◆需求定稿 |
| S1 方案 | 方案 / 计划 / **验证判据表 V1…Vn** | 产品方案评审 | 可行性会签 ◆方案与交付物确认 |
| S2/S3 设计 | 原型稿 / 页面规格 | 交互质检 | D1 设计输入 → D2 原型规格 ◆原型确认 |
| S4 技术方案 | design / 数据模型 / 契约 | 技术方案评审 | **契约与数据变更评审** |
| S5 实现 | 代码 / 迁移 / 配置 | `rd-review` | 独立代码评审（按需） |
| S6 实现一致性 | — | — | D3 锚点与截图并置 |
| S7 测试 | 验证证据（按 V1…Vn） | — | `test-verification` 盲测 |
| S8 交付 | 发布单 / 回滚方案 | 脚本内 fail-fast | **发布与环境评审** ◆放行上线 |
| S9 运营 | 指标 / 故障模式 | — | 复盘回流为判据 |

三条判定原则：

1. **自查 ≠ 独立评审**。自查是生产者自过（下限）；独立评审须**盲审隔离 + 独立上下文 + 判据外部化**，缺一即退化为自审。
2. **人审节点固定 4 个**（不过多、不过少）；AI 无权替自己确认任何一件。
3. **分级只压缩过程仪式**，不豁免「交付物定义 + 验证判据先行」两件套（开工前置不变量，见 AGENT.md）。

底层思考工具：`rd-plan/references/thinking-checklist.md`（苏格拉底辨证 / 第一性原理 / 芒格）。
红线的执行入口也在此挂载：兜底红线（无业务定义即显式报错）落在两清单的 A 项；辨证铁律（答不出 = 待确认）贯穿全程。
Anthropic 口径落点见 `../../references/anthropic-workflow-mapping.md`。
...
| 契约→开发 | `contract-reviewer` | `rd-execute` | 消费方未同步（如 import 型消费方仍用已删类型） | 交开发整改，评审不改码 |
