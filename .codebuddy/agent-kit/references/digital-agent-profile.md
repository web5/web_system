...
| **能力集** | 14 个技能（Hub + 12 子技能 + 1 项资产维护型能力，见 §三）：1 个 Hub + 4 个生产流水线（rd-execute 内含收尾完成验证门）+ 1 个设计角色 + 2 个问题处理 + 1 个探索 + 1 个审查 + 1 个记忆 + 1 项资产维护（`karpathy-llm-wiki`）；**通用前后端开发能力**：`fe-dev-common.md` / `be-dev-common.md`（业界通用技能面 + 工程质量规则，项目侧再叠加项目技能，如 web_system 的 fe/be-developer） |
...
## 三、技能集（14 个：Hub + 12 子技能 + 1 项资产维护型能力）
...
| 资产维护 | `karpathy-llm-wiki` | `raw/`（不可变源）→ `wiki/`（编译结论）知识库的建与维护：Ingest / Query / Lint；协议在 kit、数据在项目 | 不把项目数据写进 kit；不静默改写历史（冲突须标 Status 块） |
...
- **评审链**（方案质量门；模型是「**环节 × 角色**」而非一条链，细节见 `skills/rd-digital-agent/references/review-chain.md`）：
  环节 S0 需求 → S1 方案（`产品方案评审` product-review-checklist.md：问题真实性/价值优先级/成功指标/竞品惯例/需求完整性）
  → S2/S3 设计（`UX 交互质检` ux-review-checklist.md：信息架构/任务流/状态矩阵/可用性 + **D1/D2 独立设计评审**）
  → S4 技术方案（`技术方案评审` review-checklist.md：需求覆盖/架构/非功能/选型/数据接口/风险演进/可执行性）+ **契约与数据变更评审**
  → S5 实现（`代码评审` rd-review 自查）→ S6 **D3 实现一致性评审** → S7 **`test-verification` 独立盲测**
  → S8 交付（**发布与环境评审**）→ S9 运营回流为判据
  每环节内「生产者 / 自查（下限）/ 独立评审（第三方）」三者分离——**自查 ≠ 独立评审**；**人审固定 4 个**（需求定稿 / 方案与交付物确认 / 原型确认 / 放行上线）
