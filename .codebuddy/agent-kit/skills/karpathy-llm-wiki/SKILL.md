...
本技能由 `ai-agent-kit` 承载 **schema 与流程**：本文件、`references/`（模板）、`scripts/`（校验脚本）、`examples/`（样例产物）。

`raw/` 与 `wiki/` 的**数据**属于**消费项目**，不进 kit：

- 数据目录的位置、是否入库、与项目已有文档体系（如 `docs/`）的边界，由项目自行规定并在项目侧写清；
- kit 不携带任何 `raw/` / `wiki/` 内容，也不假设目录位置超出项目根。

> 边界判据：能被第二个项目原样复用 → kit 层；只对本项目成立 → 项目层。
