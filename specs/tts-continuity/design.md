...
| **P1 端到端（经 gateway）** | 首包 **466ms**，总耗时 1.42s，pcm 497856B（约 15.6s 音频）；用户实测听感「很顺」，接缝消失 → **P1 验收通过** |

## 8.1 P2 不再单独推进

P2 的目标（消除等待与不连贯）已由 P1 达成：首包 466ms 且全程连续。
端侧预热（P2-c）会带来「未点击也计费」的成本，长文本异步（P2-a）与对话式 TTS（P2-b）
分别面向离线批量与换引擎，本期均不推进。
...
### 10.3.1 实测结果（音色 603007，AppId 1254431057，文本为用户的 330 字符长句）

| 指标 | mp3 | pcm（16k/16bit/mono） |
|---|---|---|
| WS 连接 | 122ms | 123ms |
| 服务端 READY | 183ms | 247ms |
| **首包音频** | **653ms**（发文本后 470ms） | **817ms**（发文本后 570ms） |
| 整段合成完 | 4318ms | 4020ms |
| 音频 | 108920B / **30 帧** | 867744B / **113 帧** |

结论：
1. **603007 支持流式**（无 10001 报错）——P1 成立。
2. 首包 653ms **比现有「先合成首句再播」还快**，同时全程连贯 → 不必在「首播速度」与「零接缝」之间取舍。
3. 逐句发送（`ACTION_SYNTHESIS` 每句一次）确认有效，未出现文本长时间缓存不出音频。

### 10.3.2 通道与格式选型（据实测调整）

- **格式**：PC 端用 **pcm**（WebAudio 零解码、延迟最低、规避 MSE 对 `audio/mpeg` 的兼容差异）；
  小程序端保持 mp3 整段（P0 已修，省流量）——服务端 `codec` 参数可选，各取所需。
- **通道**：gateway 无 `@nestjs/websockets` 依赖（仅 `platform-express`），
  P1 一期用 **HTTP chunked 流式转发**打通 PC 端（零新依赖）；小程序端后续按 WS 或 `enableChunked` 评估。
- **端侧播放**：`fetch` + `ReadableStream` 收分片 → Int16 转 Float32 → `AudioBufferSourceNode` 排队播放（边收边播）；
  整段末尾 30ms 淡出，修复「读完突然没声音」的生硬感。
- **降级**：流式失败/超时 → 回退现有 `speakSequence`（首句 + 剩余整段）。
