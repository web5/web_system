...
## 4. R3 需求 spec：绑定邮箱（与手机号同批）

### 做成定义（一句话可验证）

> 用户在「个人信息」页点击邮箱「去绑定」后，输入邮箱并凭服务端下发的验证码完成绑定；绑定后显示脱敏邮箱，未绑定显示「去绑定」，已绑定可「更换」；邮箱命中已有账号时走与手机号**同一套合并流程**。

### 验收判据（EARS，可机器核对）

- [ ] WHEN 用户点击邮箱「去绑定 / 更换」 THE SYSTEM SHALL 进入邮箱绑定页（自绘表单，非 `wx.showModal`——原生弹窗无输入框）
- [ ] WHEN 用户输入邮箱并点「获取验证码」 THE SYSTEM SHALL 先做格式校验，格式错误时给出**字段级**提示（不刷新整页、不清空输入）
- [ ] WHEN 验证码发送成功 THE SYSTEM SHALL 启动 **60s 重发倒计时**，倒计时内「获取验证码」不可用
- [ ] WHEN 服务端收到验证码发送请求 THE SYSTEM SHALL 做限频（同一邮箱 60s 内不可重发、单日上限），超限返回 429 并给出可识别文案
- [ ] WHEN 用户提交正确验证码 THE SYSTEM SHALL 校验通过并把邮箱写入当前用户 `email` 字段（验证码只在服务端校验，不下发前端）
- [ ] IF 验证码错误 THE SYSTEM SHALL 提示错误并保留已输入邮箱；**连续错误达上限（5 次）**后该验证码作废，需重新获取
- [ ] IF 验证码过期 THEN THE SYSTEM SHALL 提示「验证码已过期，请重新获取」
- [ ] WHEN 绑定成功 THE SYSTEM SHALL 返回脱敏邮箱（如 `a***@example.com`）并即时刷新「个人信息」页展示
- [ ] IF 该邮箱已被其他账号绑定 THEN THE SYSTEM SHALL 返回可识别冲突（409 `EMAIL_ALREADY_BOUND`）并走与手机号**同一套合并流程**（二次确认 + 合并不可撤销 + 凭证重签）
- [ ] IF 当前用户已绑定邮箱 THEN THE SYSTEM SHALL 提供「更换」，走同一验证流程覆盖旧值（确认文案写明当前邮箱将被替换）
- [ ] IF 请求未带有效 access token THEN THE SYSTEM SHALL 返回 401，不写库
- [ ] IF SMTP 未配置 / 发送失败 THEN THE SYSTEM SHALL 返回 503 + 「邮件服务暂不可用，请稍后再试」，不得假装发送成功

### 边界 / 反例（不算做成）

- 反例1：继续留 `bindEmail()` 的 toast 占位 → 不算做成（`pages/mine/profile/profile.ts:21`）
- 反例2：把验证码返回给前端、由前端判断对错 → 不算做成（验证码属凭证，只服务端校验）
- 反例3：邮箱不做唯一校验，两个账号绑同一邮箱 → 不算做成（`users.email` 已有 UNIQUE 索引）
- 反例4：邮箱明文全量展示在「我的」页 → 与手机号口径不一致，须脱敏
- 反例5：无重发限频（可被用来刷邮件 / 当垃圾邮件跳板）→ 不算做成
- 反例6：邮箱绑定做了，但冲突时不走合并（与手机号两套逻辑）→ 不算做成

### 必然失败清单

- 做法A（错）：在小程序里用 `wx.showModal` 收邮箱 → 规避：原生弹窗无输入框，必须自绘页面/组件
- 做法B（错）：验证码存明文、不过期、不限制错误次数 → 规避：存 hash + 有效期 + 错误计数
- 做法C（错）：邮箱冲突另写一套分支 → 规避：合并流程抽成公共能力，手机号与邮箱都传 `bindSource` 复用

### 待确认项

- [ ] Q17：是否需要「解绑邮箱」？（手机号不可解绑，邮箱通常允许）— 本 spec 一期不做，列 P2
- [ ] Q18：邮箱是否用于登录（邮箱 + 验证码登录）？— 一期仅作打通键，不提供登录
- [ ] Q19：SMTP 凭据是否已在 dev/生产配置（`SMTP_HOST` 等）？未配置则接口直接 503
