...
> **状态基准**：2026-09-24 —— P0/P1/P2 已全部合并（`master = fac1c41`），open PR 已清空，自检 `PASS=22 FAIL=0`。
...
### A1 · spec 回写同步 —— 文档已陈旧（零风险，可立即做）

`design.md` 的 §4 落地清单与 §7 决议记录仍写着「未落 / 阻塞 / 待拍板」，而实际全部已合并。**陈旧状态会误导接手者**。
...
| # | 项 | 状态 |
|---|---|---|
| B1 | 配置中心 / `INTERNAL_API_KEY` 下发（方案 1） | 已提交 `663aa2e`，随 **PR #182** 待合并 |
| B2 | 小程序账号能力待办清单 F1–F20 | 已提交 `0be5c56`，文档在 `docs/development/mp-account-follow-ups.md`，随 **PR #182** 待合并 |
| B3 | `feature/mp-account` 分支 | #182 合并后应切回 `master` 并 pull（该分支已两次合并，继续当检出分支会持续累积） |
