...
> 建立：2026-09-24 ｜ 状态：决策已确认，可执行 ｜ 范围：先 local 验证，dev/prod 后续
>
...
| Q3 | `PIPELINE_WORKER_CONCURRENCY` 默认值 | **✅ 已决策（2026-09-24）：2**（详见 §12） |
| Q4 | 是否把 `publish-*.sh` 纳入本期 | **✅ 已决策（2026-09-24）：不纳入**——列后续 V11，本期在 runbook 记录禁用约定 |
| Q6 | CI 侧 `concurrency` group 何时放宽 | **✅ 已决策（2026-09-24）：本期交付并跑通后再放宽** |
| Q8 | `p29` 是否保留 `DRY_RUN` | **✅ 已决策（2026-09-24）：保留**——Q5 取消 ROLLBACK 后的唯一事前防线 |
| Q9 | V6 判定阈值是否接受 | **✅ 已决策（2026-09-24）：接受**——不达标应改实现方式，不放宽阈值 |
| Q10 | T4b 前端排队态本期是否做 | **✅ 已决策（2026-09-24）：不做**——V1/V2 不依赖它，UI 改动另走动作门 |
...
## 12 决策结果汇总（2026-09-24 全部确认）

> 本节是执行入口的**唯一配置清单**。至此无待确认项阻塞，可开工。

| 项 | 决策 | 落实位置 |
|---|---|---|
| Q1 | **不分期**，一次性实现、一次性验收 | `design.md` §6；`tasks.md` 单一交付点 |
| Q2 | 工作目录 `<ws>/.runs/<pipelineId>` | §4.3.2；`tasks.md` T6 |
| Q5 | `deploy_release_locks` **一次性替换并 DROP**，不提供 `ROLLBACK` 脚本 | §4.2.4；`tasks.md` T2 + T0 快照（G0） |
| N1 | **不引入运行时开关 / 兼容模式**；回滚 = `git revert` + T0 快照 | §6.3 / D7；`tasks.md` T10；判据 V7′ |
| H1–H7 | 顺手拆除，作为各任务同行义务 | §6.4；`tasks.md` T2/T3/T5/T7/T8；判据 V13 |
| Q3 | `PIPELINE_WORKER_CONCURRENCY` = **2** | `tasks.md` T3 |
| Q4 | `publish-*.sh` **不纳入**本期；runbook 记「并发发布期间禁止手工执行」 | `tasks.md` T10；V11 留后续 |
| Q6 | CI `concurrency` group **本期交付跑通后再放宽** | §6.5 后续 |
| Q8 | `p29` **保留 `DRY_RUN`** | §11 Q8；`tasks.md` T1；判据 V8 |
| Q9 | **接受** V6 阈值（增量 ≤ 源码 ×1.2、硬链占比 ≥ 90%） | `requirements.md` V6 |
| Q10 | T4b 前端排队态 **不做** | §6.5 后续；UI 改动另走动作门 |

**执行顺序**：T0 → T1 →（T2/T3/T4a 与 T5/T6/T7 可并行）→ T8 → T9/T10 → T11 → T12。
**门禁**：G0 快照 → G1 迁移幂等 → G2 隔离生效（V3 硬门禁）→ G3 可恢复（V7′）→ G4 无包袱（V13）→ G5 收尾。
