...
> 建立：2026-09-24 ｜ 状态：待用户确认 ｜ 范围：local 验证
...
| **T3** | 队列与 worker pool | `PipelineQueue`（enqueue / claim / complete）；进程内 worker loop；`PIPELINE_WORKER_CONCURRENCY`（默认 2）；启动 reconcile（孤儿 run 重置为 `queued`、过期 lease 回收） | **H7**：删除后台 fire-and-forget `execAsync git fetch` | T2 | **V1, V2, V13** |
...
| **T10** | 回滚说明（**不是灰度开关**） | 写进 `docs/development/local-release-runbook.md`：`git revert` + T0 快照恢复两步动作、适用环境、耗时预期 | **H1**：**不引入** `PIPELINE_WORKSPACE_MODE` 或任何运行时兼容模式 | T5 | **V7′** |
