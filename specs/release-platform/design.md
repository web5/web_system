...
### 3.1) R6 落地设计（2026-09-10 用户决策确认）

三项落地口径（用户拍板，**替代原 R6 的"过渡期双读"路径**）：

| # | 决策 | 含义 |
|---|---|---|
| D-a | **版本/产物身份纳入流水线维度** | 产物命名空间 = (模块 × 流水线 × 版本)，不再是 (模块 × 版本) |
| D-b | **只保留新机制（硬切，不双读）** | 脚本真相源只有「流水线步骤命令」一套；模块级 `deploy_module_stage_commands` 直接下线，不留兼容回退读路径 |
| D-c | **不做跨流水线冲突治理** | 多条流水线作用于同一模块导致的互相影响由使用者自负；不建互斥锁、影响面提示、漂移告警 |

#### 产物布局与版本标识

```
产物目录   modules/<moduleKey>/<pipelineKey>/<version>/
HTTP      /static/modules/<moduleKey>/<pipelineKey>/<version>/index.js
指针值    deploy_deployments.current_version = "<pipelineKey>/<version>"
```

- `version` 仍是 git 短哈希（可读性、回滚语义不变），**流水线维度由目录层级承载**，不污染版本字符串。
- **gateway / nginx 零改动**：gateway 只做 `` `/static/modules/${moduleKey}/${version}/` `` 字符串拼接（`version` 含 `/` 时自然多出一级）；nginx 是 `location /static/modules/` + `alias` 直出，支持任意深度。新增契约：`current_version` 必须等于产物相对路径。
- **解析收口**：`release-paths` 新增 `parseReleaseRef(ref) → { pipelineKey, version }` 纯函数；所有「引用 → commit / 目录 / URL」的解析必须走它，禁止再手拼（沿用 V6「平台路径知识单点」原则）。
- **legacy 命名空间**：历史产物（`modules/<moduleKey>/<commit>`，一级目录含 `index.js`）保留，仅用于切指针回滚（指针值即目录名），不再新增。**一级目录无 `index.js` = 流水线命名空间**，`listVersions` / `cleanup` 依此区分，无需读旧脚本即可共存。
- `artifact-store` 的 root 由 `modules/<module>` 下沉为 `modules/<module>/<pipelineKey>`：保留 N=5、清理、复用产物判定均在**流水线命名空间内**进行（各流水线各自保留 5 个版本、各自回滚候选清单）。
- **指针仍是模块级唯一**：`deploy_deployments` 是 `UNIQUE(envId, moduleKey)`，gateway 只按 (环境, 模块) 读版本。多条流水线写同一模块 = 后写覆盖——这正是 D-c 明确接受的代价（产物物理隔离，线上指针不做仲裁）。

#### 现有模块命令的迁移（D-b 的硬前置）

硬切后 `build` 是 fail-fast（未配命令即终止发布），所以**必须先迁移数据再切读路径**，否则全部模块发布停滞：

1. 对每个「已配置模块级命令」的模块，自动生成一条**专用流水线**（复制全局默认 + 把其 `deploy_module_stage_commands` 逐阶段写为对应节点的操作序列）。模块 ↔ 专用线一一对应 → 语义与现状等价，无需 `{MODULE_TYPE}` 分支。
2. 全局「默认」线服务无自定义命令的模块，其 build 用 `{MODULE_TYPE}` 变量分支模板（见 §2「差异表达」）。
3. 现有 hook 化脚本（content-hub / upload-service / ai-agent 的 build，admin / portal 的 build + upload + cleanup）随专用线一并迁移。
4. 读路径替换：`runStageCommand(p.moduleKey, stage)` → `resolveStepActions(p.pipelineKey, nodeKey)`；随后删除 `deploy_module_stage_commands` 表与 `StageCommandService` 的模块维度（保留 `isWritableStageKey` 等纯校验）。
5. **连带面**（迁移时必须一并改，否则功能断链）：`publish_version` / `rollback` / `list_releases` 的回滚候选需按流水线分组 → MCP 工具签名与前端版本候选列表需带 `pipelineKey`；`cleanup` 的「保留 N 个」作用域变为流水线级。

#### 明确放弃的护栏（D-c）

- 不做「同一模块被多条流水线写入」的互斥锁、影响面提示、命令漂移巡检。

#### 与既有条目的冲突修正

- **R2 的「模块页仍叫阶段命令」作废**：D-b 后模块不再持有命令，模块页的「阶段命令」tab 整体移除，命令编辑只存在于流水线节点内（上一版设计里节点配置的「作用模块」下拉随之取消）。
- **R6「过渡期兼容回退」作废**：改按 D-b 硬切（迁移 → 下线 → 一步切换），不做双读。
- 历史踩坑教训（"两套互斥机制并存"）在此以**先迁移后切换**的顺序消化，而不是靠双读兼容。

#### 落地口径确认（2026-09-10，原型评审通过）

原型：`docs/ui/prototypes/release-platform-v6-pipeline-scripts.html`；页面规格书：`specs/release-platform/ui-spec-v6-pipeline-scripts.md`。
八项口径全部确认（用户"按推荐来"）：

| # | 确认结论 |
|---|---|
| 1 | 产物命名空间 = `modules/<模块>/<流水线key>/<版本>/`（目录分层）；gateway / nginx **零改动** |
| 2 | 流水线新增 `key`（slug，`^[a-z0-9-]{1,32}$`，全局唯一）；内置全局默认线 key = `default` |
| 3 | 流水线 key **允许改名**：历史产物按旧 key 目录留存、仍可回滚；改名时提示「新产物将落在新命名空间」 |
| 4 | 默认线的模块差异用命令内 `{MODULE_*}` 变量 + 「按模块预览」表达；不内置 `case` 分叉模板；差异大 → 复制专用线 |
| 5 | 模块页「阶段命令 / 发布脚本」tab **整体移除** → 改为提示条 + 「查看引用本模块的流水线」跳转，不保留只读总览 |
| 6 | 回滚候选**按流水线命名空间分组**渲染 + 独立 legacy 组，不做跨组混合排序 |
| 6b | **IA 调整（2026-09-10 反馈，v7 原型）**：① 发布环境选择收进**流水线详情**「发起发布」，取消独立的「提交流水线」菜单/页面；② 「版本与回滚」收进**流水线详情**，不做独立菜单；③ **模块页只展示「环境 × 当前版本」**，不标注「所属流水线」——当前版本可能是多条流水线交叉发布的结果，标注归属会误导；④ 模块页「发布脚本」tab 仍整体移除 |
| 6c | **IA 调整（2026-09-10 追加反馈，v8 原型）**：① 「流水线」菜单默认落地**全部流水线列表**，点列表项进详情；② 详情 tab 顺序 = **当前运行（最新一次实例）→ 执行历史 → 编排与命令 → 发起发布**；③ **取消独立的「版本与回滚」tab**：版本信息与**回滚操作下沉到「执行历史」行内**（legacy 历史产物表跟随其后）；④ 回滚只切本流水线命名空间内的版本（跨线版本去对应流水线） |
| 6d | **IA 调整（2026-09-10 终版，v9 原型）**：**运行态与定义态分离** —— ① 详情页只留运行态 tab（当前运行 / 执行历史）；② 「编排与命令」收进页头「**编辑**」按钮打开的**编辑抽屉**（三段：基本信息 / 流程编排 / 节点命令，底部保存 + 未保存关闭确认）；③ 「发起发布」同样为页头按钮打开的抽屉（环境 tabs + 模块 + 分支/commit + 模式 + 身份预览）；④ 详情页页头右上 = 「编辑」（default）+「发起发布」（primary ≤1）；⑤ 日志查看为独立抽屉（阶段切换） |
| 6e | **IA 调整（2026-09-10 最终，v10 原型）**：**「编辑」改为独立页面**（非抽屉）—— ① 路由 `/pipelines/:id/edit`，面包屑「‹ 返回流水线详情」返回；② 页头右侧 = 删除流水线（禁用 + tooltip）/ 取消 / 保存（primary）；③ 三段内容（基本信息 / 流程编排 / 节点命令）同 6d；④ 未保存离开（取消 / 面包屑 / 侧栏切换）统一弹「放弃修改」确认；⑤ 发布与日志仍用抽屉（短停留操作），编辑用页面（长停留定义态） |
| 6f | **IA 调整（2026-09-10 最终，v11 原型）**：**发布环境选择改用下拉**（`a-select`，不预置枚举、选项来自环境管理）——环境是动态集合、数量不固定（本地/dev/test/stage/prod/…），tabs 平铺不可扩展；选中后身份预览即时联动。R2 规则的适用例外：动态选项（>5）适用下拉 |
| 6g | **IA 调整（2026-09-10 最终，v12 原型 + 本轮细化）**：**目标模块按入口上下文联动**——① **流水线列表行「发起发布」** → 模块**可选**（不受 scope 限制，让用户选）；② **详情页头 / 执行历史「再次发布」 / 当前运行「再次发布」 / 模块页**入口 → 模块**默认选中且禁用**（禁用 + 「已锁定」tag + 旁注，默认 = admin 或来源实例/模块的模块）；③ **专用线**的详情页头 / 记录入口 → 下拉只含其适用模块且直接锁定（scope 过滤） |
| 7 | 历史产物 = **legacy 命名空间**：可切指针回滚、不再新增；「保留最近 5 个」按命名空间各自计 |
| 8 | 迁移：为每个「已配命令」的模块**自动生成专用线**（模块 ↔ 专用线一一对应，语义等价）；未配模块走默认线；迁移后下线模块级表与读路径 |
