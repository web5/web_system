...
「模型」页重构为**单表**，详见 `specs/llm-models-unify/page-spec.md`。要点：

- 一张表展示：模型 id、展示名、提供方、输入价/1K、输出价/1K、状态、排序、操作（编辑 / 启停 / 删除）；
- 新增模型、改价、启停都在此页完成（底层调 `admin/dict` 接口，不再调 `admin/model-pricing`）；
- 「字典管理」里 `llm_models` 行加提示标签「由『模型』页维护」，点进去仍可编辑（高级视图，不作主入口）；
- 顶部一致性提示：`未配单价的启用模型 N 个`（可点击筛选），风格同「角色权限」页的差异提示。
...
7. **页面归属**：「字典管理」里的 `llm_models` 是「加提示但保留可编辑」，还是直接隐藏入口？
