...
> 决策：monorepo 内 `packages/kedou-agent`；抽公共核心包 `@kedou/agent-core`；增加 web-search 与 coding 工具；生图工具不放入包；无 demo。
...
### 3.5 代码共享
- When 项目构建，ai-service 与 `kedou-agent` 应通过 `@kedou/agent-core` 共用 harness 核心（引擎/注册表/接口/工具）。
...
- 公共核心包 `@kedou/agent-core`（纯 TS，零 Nest，harness 核心）
