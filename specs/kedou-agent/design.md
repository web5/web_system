...
```
packages/
├── agent-core/            # 🆕 公共核心包 @kedou/agent-core（纯 TS，零 Nest，可发布）
│   ├── package.json       # name: @kedou/agent-core
│   ├── tsup.config.ts
│   └── src/
...
**关键设计**：`@kedou/agent-core` 是两端共享的核心（ai-service 通过它跑 Agent，CLI 也通过它跑 Agent）。ai-service 在其外层保持 Nest 装饰器 + DB `ConversationMemory` + controller，但**引擎/注册表/客户端/工具逻辑来自 agent-core**。
...
1. `@kedou/agent-core` 骨架（interfaces + lib/logger + lib/timeout）
