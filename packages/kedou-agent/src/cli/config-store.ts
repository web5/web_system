...
const SEARCH_ENV_KEYS = ['BING_SEARCH_API_KEY', 'BOCHA_API_KEY'];
...
export function listSearchKeyStatus(): Array<{ id: string; available: boolean }> {
  return SEARCH_ENV_KEYS.map((k) => ({ id: k, available: !!process.env[k]?.trim() }));
}
